      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {sriLankaPackages.map((pkg) => (
            <div
              key={pkg.id}
              className="group bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 hover:translate-y-[-4px] flex flex-col h-full"
            >
              {/* Image Container */}
              <div className="relative h-64 w-full overflow-hidden">
                ...
                <div className="absolute top-4 right-4 bg-white/90 backdrop-blur px-3 py-1 rounded-full text-xs font-bold text-slate-800 flex items-center gap-1 shadow-sm z-10">
                  <svg
                    className="w-3.5 h-3.5 text-brand-accent fill-brand-accent"
                    viewBox="0 0 20 20"
                  >
              ...
              <div className="p-6 flex flex-col flex-grow space-y-4 text-left">
                <div className="flex justify-between items-start gap-2">
                  <div>
                    <h3 className="text-lg font-bold text-brand-dark group-hover:text-brand-primary transition-colors leading-tight">
                      {pkg.name}
                    </h3>
                  ...
                  <div className="text-right">
                    <span className="block text-xxs font-bold text-slate-400 uppercase tracking-wider">From</span>
                ...
                <div className="space-y-1">
                  <span className="block text-xxs font-bold text-slate-400 uppercase tracking-wider">Destinations</span>
                  <div className="flex flex-wrap gap-1.5 pt-0.5">
                    {pkg.destinations.map((dest, i) => (
                      <span key={i} className="inline-flex bg-slate-100 text-slate-700 text-xs px-2 py-0.5 rounded-md font-semibold">
                        {dest}
                      </span>
                    ))}
                  </div>
                ...
                <div className="space-y-1 flex-grow">
                  <span className="block text-xxs font-bold text-slate-400 uppercase tracking-wider">Includes</span>
                  <ul className="text-xs text-slate-500 font-semibold space-y-1 pt-1 pl-4 list-disc">
