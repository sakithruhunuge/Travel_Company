    ...
    return (
        <div className="rounded-2xl bg-white/60 backdrop-blur-sm p-6 border border-slate-200 transition-transform hover:-translate-y-0.5">
            ...
            <div className="mt-5 grid gap-4 text-sm sm:grid-cols-2">
                <div>
                    <p className="text-xs font-medium uppercase tracking-wider text-brand-muted">Travelers</p>
                    <p className="mt-1 tabular-nums font-medium text-brand-dark">{request.numberOfTravelers}</p>
                </div>
                <div>
                    <p className="text-xs font-medium uppercase tracking-wider text-brand-muted">Preferred date</p>
                    <p className="mt-1 tabular-nums font-medium text-brand-dark">
                        {new Date(request.preferredStartDate).toLocaleDateString()}
                    </p>
                </div>
                <div>
                    <p className="text-xs font-medium uppercase tracking-wider text-brand-muted">Submitted</p>
                    <p className="mt-1 tabular-nums font-medium text-brand-dark">
                ...
                <div className="mt-5 flex flex-wrap gap-3 border-t border-white/10 pt-5">
                    ...
                    {request.status === "pending" && onCancel ? (
                        <button
                            ...
                            className="rounded-md bg-rose-50/80 border border-rose-200 px-4 py-2 text-sm font-medium text-rose-700 transition-colors duration-200 hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-50"
                        >
